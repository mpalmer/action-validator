...
pub fn run(config: Config) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let file_name = config.src.file_name().ok_or("Unable to derive file name from src!")?;
    let valid_doc = if file_name == "action.yml" || file_name == "action.yaml" {
        if config.verbose {
            eprintln!("Treating {} as an Action definition", config.src.to_str().ok_or("Unable to convert PathBuf to string!")?);
        }
        validate_as_action(&doc)
    } else {
        if config.verbose {
            eprintln!("Treating {} as a Workflow definition", config.src.to_str().ok_or("Unable to convert PathBuf to string!")?);
        }
